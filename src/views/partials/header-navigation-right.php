...
<?php
    ...
    foreach($main_menu as $key => $fi) : 
        ...
        <?php } else { 
            ...
            <div class="has-sub-menu <?= \SDEV\Utils::getMenuItemClass( (rtrim(get_permalink(), '/').'/'), $menu_item->url) ?>">
                <a href="<?= $parent_url; ?>" data-id="<?= $menu_item->ID; ?>" class="<?= $classes ?>"><?= $menu_item->title; ?></a>
                <ul class="sub-nav">
                    <?php foreach($fi['submenu'] as $sub) :
                        ...
                        <li><a href="<?= $sub->url; ?>" target="<?= $sub->target ?>" class="<?= $classes ?> <?= \SDEV\Utils::getMenuItemClass( (rtrim(get_permalink(), '/').'/'), $sub->url) ?>"><?= $sub->title; ?></a></li>
                    <?php endforeach; ?>
                </ul>
            </div>
        <?php } 
